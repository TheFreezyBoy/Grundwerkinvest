...
const PropertyDetailPageClient = ({ property: propertyData }) => {
  const [currentImageIndex, setCurrentImageIndex] = useState(0)
  const [showModal, setShowModal] = useState(false)
  const [showLightbox, setShowLightbox] = useState(false)
  const [lightboxImageIndex, setLightboxImageIndex] = useState(0)
  const [investmentAmount, setInvestmentAmount] = useState(propertyData.price)
      ...
      <section className="bg-gradient-to-br from-primary to-secondary py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 text-white/90 text-lg">
            ...
            <span>
              {propertyData?.location}, {propertyData?.city?.name}
            </span>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-4 gap-4">
            {/* Large Featured Image - Takes 2x2 Grid */}
            <button
              onClick={() => openLightbox(0)}
              className="col-span-2 row-span-2 relative group rounded-2xl overflow-hidden border-2 border-border hover:border-accent transition-all shadow-lg hover:shadow-2xl"
            >
              <ImageWithFallback
                src={propertyData?.images?.[0]?.url}
                alt={`${propertyData?.title} - Main`}
                className="w-full h-full object-cover aspect-square"
              />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity bg-accent text-accent-foreground rounded-full p-3 shadow-xl">
                  <Maximize2 className="w-6 h-6" />
                </div>
              </div>
            </button>

            {/* Secondary Images - Remaining Grid Spaces */}
            {propertyData?.images?.slice(1, 6)?.map((image, index) => (
              <button
                key={index + 1}
                onClick={() => openLightbox(index + 1)}
                className="relative group rounded-2xl overflow-hidden border-2 border-border hover:border-accent transition-all shadow-lg hover:shadow-xl"
              >
                <ImageWithFallback
                  src={image?.url}
            ...
            {/* "View All Photos" Button if there are more images */}
            {propertyData?.images?.length > 6 && (
              <button
                onClick={() => openLightbox(0)}
                className="relative group rounded-2xl overflow-hidden border-2 border-accent bg-gradient-to-br from-primary to-secondary hover:from-primary-hover hover:to-secondary-hover transition-all shadow-lg hover:shadow-xl"
              >
                <div className="w-full h-full aspect-square flex flex-col items-center justify-center gap-2 text-white">
                  <Maximize2 className="w-8 h-8 text-accent" />
                  <span className="text-lg">View All</span>
                  <span className="text-sm text-white/80">
                    {propertyData?.images?.length} Photos
                  </span>
                </div>
              </button>
            )}
              ...
              <div className="bg-card rounded-2xl border border-border p-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6 pt-6 border-t border-border">
                  ...
                  <div>
                    ...
                    <div className="text-xl text-foreground">{propertyData?.city?.name}</div>
                  </div>
                  <div>
          ...
          <div className="relative h-full flex flex-col">
            ...
            <div className="absolute top-0 left-0 right-0 z-10 p-6 bg-gradient-to-b from-black/80 to-transparent">
              <div className="flex items-center justify-between">
                <div className="text-white">
                  <h3 className="text-xl mb-1">{propertyData.title}</h3>
                  <p className="text-sm text-white/70">
                    Photo {lightboxImageIndex + 1} of {propertyData.images.length}
                  </p>
                </div>
                <button
            ...
            <div className="flex-1 flex items-center justify-center p-20">
              <div className="relative max-w-6xl max-h-full">
                <ImageWithFallback
                  src={propertyData.images[lightboxImageIndex]?.url}
                  alt={`${propertyData.title} - ${lightboxImageIndex + 1}`}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent">
              <div className="flex gap-3 justify-center overflow-x-auto pb-2 pt-2 scrollbar-hide">
